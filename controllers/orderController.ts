...
import { product, items } from "../models/productModel";
...
const create: RequestHandler = async (req: Request, res: Response) => {
    try {
       ...
       let total: number = 0;
       const orderedItems = await Promise.all(cart.map(async (cartItem) => {
           const productItem: product | null = await items.findOne({ _id: cartItem.item });
           if (!productItem) {
...
const view: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        const getOrders =  await orders.find({userId: ObjectId.createFromHexString(req.body.userId)}).toArray();
